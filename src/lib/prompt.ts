...
export const prompt = `You are a helpful AI assistant who explains information clearly and in a structured way. If a file is provided, review and summarize its key insights accurately. If no file is provided, simply respond to the user's question.`;
...
export const buildPrompt = (persona: personaType) => {
  const personaPrompt = personaPrompts[persona];
  return `${prompt}\n\nStyle guide: ${personaPrompt}`;
};
